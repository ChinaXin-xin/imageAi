...
- 上传实拍图、排版图、壁纸图，并使用 GPT 深析可见产品细节。
...
针对三星 S23U / S23 Ultra 镜头膜，最终提示词会额外锁定右侧三个小孔的大小差异，避免模型把它们生成成等大孔。最终提示词也会加入物品白名单，只允许出现手机/手机模型、上传图对应屏幕膜、上传图对应一体式镜头膜和已选择套装配件，禁止额外黑色小包、收纳袋、包装盒、卡片、托盘等未选择物品。
...
场景规划进入生图前会做一轮自审查：如果 GPT 场景里出现额外包装、包装盒、小黑包、收纳袋、卡片、托盘等未选择物品，后端会丢弃该场景并使用安全备用场景。多图任务会强制混合 3D 立体斜角、平铺俯拍、近景细节等不同角度，避免所有图片都变成平面摆拍。
