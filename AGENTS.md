...
- 任务上传的排版图、Logo 图、壁纸图必须分别支持选择用于主图或介绍图，默认主图和介绍图都使用；后端按用途拼接排版图风格提示词，并按用途传递 Logo 图和壁纸图参考图。
- 任务上传的 Logo 图和壁纸图不做 GPT 深析或自动识别，生图时按已选用途作为参考图直接贴用；Logo 不改字、不重绘，壁纸直接贴到手机屏幕或屏幕展示区域。
- 清洁/安装辅助配件不能写死形状、颜色、尺寸或文字；不同手机型号和套装的配件形态可能不同，必须按上传参考图和深析结果生成。若参考图配件上有文字，最终图只保留参考图可见文字，不要凭空加字、改字或生成无字替代品。
...
- 排版模板图片只用于前置风格分析，生图时只拼接对应类型的排版模板风格文字，不把排版模板原图作为生图参考图传给模型。
