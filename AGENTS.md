...
- 最终生图提示词按“真实产品结构锁定/参考图优先级 → 上传图深析结果 → 手机膜/镜头膜结构生成规则 → 套装规格数量锁定 → 任务参数/规格 → 主图/介绍图画面要求 → 参考风格图风格 → 负面约束/生成要求”的顺序拼接；不要把深析上传图的提示词本身拼进最终生图提示词。
...
- 任务上传入口不再提供“包装图”；前端和后端任务创建只保留实拍图、参考风格图、Logo 图、壁纸图以及已选择的额外配件/参考风格图。
- 任务上传的参考风格图、Logo 图、壁纸图必须分别支持选择用于主图或介绍图，默认主图和介绍图都使用；后端按用途拼接参考风格图风格提示词，并按用途传递参考风格图、Logo 图和壁纸图参考图。
...
- 用户界面统一称为“参考风格图”；历史代码/API/数据库字段可继续沿用 `targetTemplate`、`template`、`排版图` 等内部命名以兼容旧数据。
- 参考风格图分为主图和介绍图两类；主图参考风格图只能拼入主图生图提示词，介绍图参考风格图只能拼入介绍图生图提示词。
- 参考风格图图片仍需用于前置风格分析；生图时按主图/介绍图类型拼接对应参考风格图风格文字，并把对应参考风格图作为低优先级布局参考图传给模型，但不得让参考风格图覆盖实拍产品结构、配件数量、孔位锁定和白名单约束。
...
- 前端上传 AVIF 等非常规图片格式时，应优先在浏览器端转成 PNG 后再提交；后端分析图片时遇到 ImageIO 无法解码的图片，要保留原始 data URL 兜底，避免参考风格图上传直接 500。
