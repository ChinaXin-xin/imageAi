...
- 镜头膜和手机膜支持多品牌、多型号，不能把某个型号（如 S23U）的孔位结构写死成通用规则；必须先根据上传图和深析结果智能识别当前机型，再锁定对应外轮廓、孔位数量、位置、大小差异和一体式/分离式形态。
...
- 当前客户可用产品只有钢化膜、防窥膜、镜头膜；可用配件只有无尘布、酒精清洁包、定位神器、刮板、安装辅助贴、镜头膜安装辅助贴防滑垫。除任务已选择或上传的这些物品外，不要生成包装盒、收纳袋、卡片、托盘、支架、底座、展示道具或其他赠品。
- 酒精清洁包/湿巾包只能在任务已选择或上传参考图时出现，且必须按参考图形态生成；如果参考图袋子上有文字，最终图必须保留可见文字（如 `WET WIPES`、`Remove Dust`、`Effective Sterilization`），不要生成空白白色小袋、无字黑色小袋或任意非参考图袋子。
