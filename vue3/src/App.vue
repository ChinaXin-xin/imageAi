...
const DEFAULT_SCENE_PROMPT = `1. 3D立体斜角安装态：手机完整入画，屏幕膜/钢化膜与手机分层错位展示，有厚度、阴影和空间纵深。
...
async function loadPromptSettings() {
  ...
  try {
    ...
    const normalizedSettings = {
      ...
      scenePrompt: settings.scenePrompt?.trim() || DEFAULT_SCENE_PROMPT,
            ...
            <section class="task-card settings-card" v-loading="settingsLoading">
              ...
              <div class="form-row">
                ...
                <el-input
                  ...
                  maxlength="3000"
                  show-word-limit
                  placeholder="请输入多张图的场景/卖点方向；建议每行一条，后端会按顺序分配到生成图"
                />
              </div>
              ...
              <article class="analysis-card">
                ...
                <div v-else class="scene-list">
                  <div v-for="scene in selectedQueueTask.mainScenes" :key="`main-${scene.index}`" class="scene-item">
                    <strong>主图 #{{ scene.index }} 场景{{ scene.index }}</strong>
                    <p
              ...
              <article class="analysis-card">
                ...
                <div v-else class="scene-list">
                  <div v-for="scene in selectedQueueTask.introScenes" :key="`intro-${scene.index}`" class="scene-item">
                    <strong>介绍图 #{{ scene.index }} 场景{{ scene.index }}</strong>
                    <p
